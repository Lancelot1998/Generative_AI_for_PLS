...
DATA_DIR = os.path.join(ROOT, "data", "europarl")
...
import numpy as np
from utils_jyz import SNR_to_noise, initNetParams, train_step, val_step, train_mi
from dataset import EurDataset, collate_data
from models_jyz.transceiver_jyz import DeepSC
from models_jyz.mutual_info import Mine
...
parser.add_argument('--checkpoint-path', default='/models/Qwen/lyq_data/checkpoints/deepsc-Rayleigh', type=str)
    ...
    for k in k_r:
        ...
        for epoch in range(args.epochs):
            ...
            if avg_acc < record_acc:
                if not os.path.exists(args.checkpoint_path + '_' + str(k)):
                    os.makedirs(args.checkpoint_path + '_' + str(k))
                # with open(args.checkpoint_path+'_'+ str(k) + '/checkpoint_{}.pth'.format(str(epoch + 1).zfill(2)), 'wb') as f:
                with open(args.checkpoint_path + '_' + str(k) + '/checkpoint.pth', 'wb') as f:
