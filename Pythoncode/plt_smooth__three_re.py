...
def plot_rewards_with_smoothing_three(
    ...
    rewards_3 = load_rewards_t(json_path_3)
    ...
    # ---------- 原始曲线 raw ----------
    plt.plot(episodes_1[:range_n], rewards_1[:range_n], color=color_raw_1, linewidth=0.5, label=f"{label_1} (raw)")
    plt.plot(episodes_2[:range_n], rewards_2[:range_n], color=color_raw_2, linewidth=0.5, label=f"{label_2} (raw)")
    plt.plot(episodes_3[:range_n], rewards_3[:range_n], color=color_raw_3, linewidth=0.5, label=f"{label_3} (raw)")

    # ---------- 平滑曲线 smoothed ----------
    plt.plot(episodes_1[:range_n], smooth_1[:range_n], color=color_smooth_1, linewidth=2.2, label=f"{label_1} (smoothed)")
    plt.plot(episodes_2[:range_n], smooth_2[:range_n], color=color_smooth_2, linewidth=2.2, label=f"{label_2} (smoothed)")
    plt.plot(episodes_3[:range_n], smooth_3[:range_n], color=color_smooth_3, linewidth=2.2, label=f"{label_3} (smoothed)")
...
if __name__ == "__main__":
    ...
    plot_rewards_with_smoothing_three(
        ...
        save_name="rewards_smooth_comparison_three_tt.png",
    )
